...
def cleanText(text):
    ...
    for word in cutlist:
        # text = re.sub(r'(\_|\-|\.|\+)' + word + '(\_|\-|\.|\+)', '+', text, flags=re.I)
        text = re.sub(r'(\_|\-|\.|\+)' + re.escape(word) + r'(\_|\-|\.|\+)', '+', text, flags=re.I)
    ...
    if re.search(r'[Ss][0-9]+[Ee][0-9]+', text):
        text = re.sub(r'[Ss][0-9]+[Ee][0-9]+.*[a-zA-Z0-9_]+', '', text, flags=re.S | re.I)
...
class tmdbScreenPerson(Screen, HelpableScreen, CoverHelper):
    ...
    def tmdbSearch(self):
        ...
        if json_data_person:
            ...
            if biography == "":
                json_data_person = tmdb.People(self.id).info(language=self.lang)  # 'en')
            if "biography" in json_data_person:
                biography = json_data_person['biography']
